...
//BUBBLE SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        copy(originalArray, originalArray + array_size, bubbleArray);
        t1 = high_resolution_clock::now();
        ...
        if(time_span < (duration<float>)150){
            bool equal = sortCheck(bubbleArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//INSERTION SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        copy(originalArray, originalArray + array_size, insertionArray);
        t1 = high_resolution_clock::now();
        ...
        if(time_span < (duration<float>)150){
            bool equal = sortCheck(insertionArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//SELECTION SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        copy(originalArray, originalArray + array_size, selectionArray);
        t1 = high_resolution_clock::now();
        selectionSort(selectionArray, array_size);
        t2 = high_resolution_clock::now();
        time_span = duration_cast<duration<float>>(t2 - t1);
        selectionTime[a] = time_span.count();
        if(time_span < (duration<float>)250){
            bool equal = sortCheck(selectionArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//MERGE SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        int *mergeArray;
        mergeArray = new int [array_size];
        copy(originalArray, originalArray + array_size, mergeArray);
        t1 = high_resolution_clock::now();
        mergeSort(mergeArray, 0, array_size - 1);
        t2 = high_resolution_clock::now();
        duration<float> time_span = duration_cast<duration<float>>(t2 - t1);
        mergeTime[a] = time_span.count();
        if(time_span < (duration<float>)500){
            bool equal = sortCheck(mergeArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//QUICK SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        int *quickArray;
        quickArray = new int [array_size];
        copy(originalArray, originalArray + array_size, quickArray);
        t1 = high_resolution_clock::now();
        quickSort(quickArray, 0, array_size - 1);
        t2 = high_resolution_clock::now();
        time_span = duration_cast<duration<float>>(t2 - t1);
        quickTime[a] = time_span.count();
        if(time_span < (duration<float>)400){
            bool equal = sortCheck(quickArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//HEAP SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        copy(originalArray, originalArray + array_size, heapArray);
        t1 = high_resolution_clock::now();
        heapSort(heapArray, array_size);
        t2 = high_resolution_clock::now();
        time_span = duration_cast<duration<float>>(t2 - t1);
        heapTime[a] = time_span.count();
        if(time_span < (duration<float>)200){
            bool equal = sortCheck(heapArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
...
//SHELL SORT
    ...
    for(int a = 0; a < elements; a++)
    {
        ...
        int *shellArray;
        shellArray = new int [array_size];
        copy(originalArray, originalArray + array_size, shellArray);
        t1 = high_resolution_clock::now();
        shellSort(shellArray, array_size);
        t2 = high_resolution_clock::now();
        time_span = duration_cast<duration<float>>(t2 - t1);
        shellTime[a] = time_span.count();
        if(time_span < (duration<float>)200){
            bool equal = sortCheck(shellArray, originalArray, array_size);
            if(!equal){
                return 0;
            }
            delete [] originalArray;
